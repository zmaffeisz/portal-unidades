...
function openOrderFormForRoom(roomId){const form=$('order-form'),sel=$('order-room');delete form.dataset.requestId;sel.value=String(roomId);sel.disabled=true;toggleForm('order-form',true);$('order-name').focus();}
...
async function submitOrder(e){e.preventDefault();if(!state.rooms.length)return toast('Cadastre uma sala antes de solicitar um item.','error');const form=e.currentTarget,room=$('order-room').value,name=$('order-name').value.trim(),quantity=Number($('order-quantity').value),unit=$('order-unit').value,priority=$('order-priority').value,justification=$('order-justification').value.trim();if(!room||!name||!Number.isFinite(quantity)||quantity<1||!unit||!priority||!justification)return toast('Preencha todos os campos obrigatórios do pedido.','error');form.dataset.requestId??=crypto.randomUUID();setBusy(form,true,'Adicionando...');try{const {data,error}=await runWithTimeout(sb.rpc('portal_criar_pedido',{p_request_id:form.dataset.requestId,p_sala_id:room,p_item_nome:name,p_quantidade:quantity,p_unidade_medida:unit,p_prioridade:priority,p_justificativa:justification}).single());if(error)return toast(error.message,'error');state.orders=[data,...state.orders.filter(item=>item.id!==data.id)];delete form.dataset.requestId;form.reset();$('order-quantity').value=1;$('order-room').disabled=false;toggleForm('order-form',false);renderCoordinator();toast('Item adicionado à lista de compras.');}catch(error){toast(error.message||'Não foi possível concluir o pedido.','error');}finally{setBusy(form,false);}}
